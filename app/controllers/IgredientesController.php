<?php
require_once __DIR__ . '/../model/Igredientes.php';
require_once __DIR__ . '/Controller.php';

class IgredientesController extends Controller
{
    private function storagePath(): string
    {
        return __DIR__ . '/../../storage/igredientes.json';
    }
    ...
    {
        $this->render('igredientes/index', ['igredientes' => $this->readAll()]);
    }

    public function show(int $id)
    {
        foreach ($this->readAll() as $it) {
            if (isset($it['id']) && (int)$it['id'] === $id) {
                $this->render('igredientes/show', ['igrediente' => $it]);
                return;
            }
        }
        $this->json(['error' => 'Igrediente não encontrado'], 404);
    }

    public function create()
    {
        $this->render('igredientes/create');
    }
    ...
    public function store(array $data)
    {
        ...
        $entry = ['id' => $nextId, 'nome' => $data['nome']];
        $all[] = $entry;
        $this->writeAll($all);
        $this->json($entry, 201);
    }
    ...
    public function edit(int $id)
    {
        foreach ($this->readAll() as $it) {
            if (isset($it['id']) && (int)$it['id'] === $id) {
                $this->render('igredientes/edit', ['igrediente' => $it]);
                return;
            }
        }
        $this->json(['error' => 'Igrediente não encontrado'], 404);
